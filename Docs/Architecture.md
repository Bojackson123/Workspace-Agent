...
code. The three admin commands are governed by env var
(`BOOTSTRAP_ADMIN_EMAILS`), **not** by the
...
| `tools/gmail.py` | `search_emails(query, max_results)` — Gmail query language, returns `Email ID + Subject` rows. `get_email(email_id)` — fetches a single message's From/To/Date/Subject headers and plain-text body (truncated at 20 000 chars). |
